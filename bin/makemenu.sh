...
# Draw initial Menu
draw_menu
while read -sn 1 key; do # 1 char (not delimiter), silent
    # Check for enter/space
    if [[ "$key" == "" ]]; then break; fi

    # catch multi-char special key sequences
    #read -sN1 -t 0.0001 k1; read -sN1 -t 0.0001 k2; read -sN1 -t 0.0001 k3  #Original, doesn't work.
    #read -sn1 -t 100 k1; read -sn1 -t 100 k2; read -sn1 -t 100 k3 #Kinda works but not really
    key+=${k1}${k2}${k3}
...
echo "${menu[$cur]}" > /tmp/menuselect.val
